...
func (caSINGen) Generate(seed uint64) []Pair {
...
func (ukNINOGen) Generate(seed uint64) []Pair {
	r := rand.New(rand.NewPCG(0xC0FFEE, seed))
	var inputs []string
	for i := 0; i < 60; i++ {
...
func (inAadhaarGen) Generate(seed uint64) []Pair {
	r := rand.New(rand.NewPCG(0xC0FFEE, seed))
	var inputs []string
	for i := 0; i < 80; i++ {
...
func (inPANGen) Generate(seed uint64) []Pair {
	r := rand.New(rand.NewPCG(0xC0FFEE, seed))
	var inputs []string
	for i := 0; i < 80; i++ {
		// AAAAA9999A format.
...
func (brCPFGen) Generate(seed uint64) []Pair {
	r := rand.New(rand.NewPCG(0xC0FFEE, seed))
	var inputs []string
	for i := 0; i < 80; i++ {
...
func (brCNPJGen) Generate(seed uint64) []Pair {
	r := rand.New(rand.NewPCG(0xC0FFEE, seed))
	var inputs []string
	for i := 0; i < 80; i++ {
