...
**Go version note.** The generator uses `math/rand/v2`. PCG state is stable across Go versions but the `r.IntN(n)` rejection-sampling implementation has changed between Go patch releases (observed between 1.26.1 and 1.26.3). If `corpus-lock-fresh` fails on a green local run, your toolchain may be ahead of or behind CI's `go-version: "1.26"`. Match CI by installing the same patch (`go install golang.org/dl/go1.26.X@latest && go1.26.X download`) and re-run `GO=$(go env GOPATH)/bin/go1.26.X make corpus-regen`.
